...
def run_single_utilization_point(
    ...
    for i in range(test_times):
        ...
        #print(i,"base_total_value=",base_total_value)
        ...
        for name, (cost_func, is_dyn) in strategies.items():
            ...
            for p in optimized_processors:
                ...
                e, s = get_normalization_utility_offline_stats(
                    p.tasks, p.drop_list, mode=exp_mode, grid_hyperperiod=global_hyperperiod
                )
                off_exec_sum += e
                #off_total+=s
            #print(i,"off_exec_sum=",off_exec_sum)
            agg_results[f"{name}_Off"] += (off_exec_sum / base_total_value)
            #print(off_exec_sum / base_total_value)
            # if off_exec_sum / base_total_value<=0.4:
            #     for p in optimized_processors:
            #         for tasks in p.tasks:
            #             print(p.id,"tasks=",tasks)
            # --- B. Online Phase ---
            ...
            # 计算 Online Utility
            # on_exec_val = online_stats["weighted_completed"]
    ...
    for i, name in enumerate(strat_names):
        ...
        if col_on in df.columns:
            plt.plot(df["Utilization"], df[col_on],
                     color=c, linestyle='-', marker=m, markersize=3, label=f"{name} (On)")

        # 绘制 Offline (虚线 + 空心标记/无标记)
        if col_off in df.columns:
            plt.plot(df["Utilization"], df[col_off],
                     color=c, linestyle='--', marker=m, markersize=3, alpha=0.5, label=f"{name} (Off)")
...
def main():
    ...
    u_values = [x * 0.05 for x in range(10, 18)]  # 0.6 - 0.8
