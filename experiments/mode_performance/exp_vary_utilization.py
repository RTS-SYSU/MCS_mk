...
def run_util_point(target_util: float,
    ...
    for run_idx in range(n_runs):
        ...
        # Snapshot drops after classification (for MC / HI baseline)
        drops_mc = [list(p.drop_list) for p in processors]

        # ---- Static: x = 0 in all modes ----
        ...
        accum["Static_L"] += _perf_mode(p_static, lo_all, beta, 'L')          # all active
        accum["Static_S"] += _perf_mode(p_static, lo_all, beta, 'S', drops_mc)
        accum["Static_H"] += _perf_mode(p_static, lo_all, beta, 'H', drops_mc)  # no recovery

        # ---- Proposed: LO augment → MC degrade → HI recovery ----
    ...
    if feasible == 0:
        return {"feasible": 0,
                "Static_L": float('nan'), "Static_S": float('nan'), "Static_H": float('nan'),
                "Proposed_L": float('nan'), "Proposed_S": float('nan'), "Proposed_H": float('nan')}

    return {"feasible": feasible,
            "Static_L":   accum["Static_L"]   / feasible,
