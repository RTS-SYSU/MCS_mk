...
def _worker_unified_run(args: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    if processors_base is not None:
        ...
        result["baseline"] = lo_executed_sum / base_total_value
        result["baseline_HI"] = hi_executed_sum / base_total_value

        sim_pure_online_lo = run_multicore_simulation(
            processors_base, global_hyperperiod, mode_switch_time=float('inf')
        )
        sim_pure_online_hi = run_multicore_simulation(
            processors_base, global_hyperperiod, mode_switch_time=0
        )

        result["pure_online"] = sim_pure_online_lo["utility"]
        result["pure_online_HI"] = sim_pure_online_hi["utility"]
...
def run_single_utilization_point_parallel(
    ...
    agg_results[f"baseline_HI"] = 0.0
    agg_results[f"pure_online"] = 0.0
    agg_results[f"pure_online_HI"] = 0.0
    for s in strategies:
...
def main():
    ...
    filename = f"secB_result_Ulo_{NUM_CORES}core"
