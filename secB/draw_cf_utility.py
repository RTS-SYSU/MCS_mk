...
df = pd.read_excel('secB_result_cf.xlsx')

# 定义策略和颜色 (Off 用深色，On 用浅色或带斜线)
#strategies = ['UASWC', 'LUF','Fair','HUF']
strategies = ['UASWC', 'Fair']
colors_on = ['#30617F', '#E8977A','#AED9E6', '#CCB89E']  # 你提供的基准颜色
colors_off = ['#97B0BF', '#D6ECF2', '#F3CBBC', '#E5DBCE']  # 精准计算的对应浅色
...
def plot_mode(mode_str):
    ...
    for i, strat in enumerate(strategies):
        ...
        bar2 = ax.bar(x + offsets[idx], df[col_on], width, color=colors_on[i], edgecolor='black',linewidth=0.8)
        bars_plotted_on.append(bar2)
        if strat == 'UASWC':
            labels_on.append('proposed')
        else:
            labels_on.append(f'{strat}')
        #labels_on.append(f'{strat}')
        idx += 1
    bar2 = ax.bar(x + offsets[idx], df['baseline'], width, color='gray', edgecolor='black', linewidth=0.8)
    bars_plotted_on.append(bar2)
    ...
    #ax.set_xlabel(r'$U_{avg}^{LO}$', fontsize=18)
    ax.set_xlabel(r'$cf$', fontsize=22)
    ...
    #all_handles += line
    # all_labels += ['Baseline']
    ax.legend(all_handles, all_labels, bbox_to_anchor=(0.2, 1), loc='lower left', ncol=5, fontsize=14, frameon=True)

    plt.tight_layout()
    # bbox_inches='tight' 极度重要！它保证放在图外的图例在输出 PDF 时不会被裁掉
    plt.savefig(f"secB_result_cf_utility_{mode_str}.pdf", format='pdf', bbox_inches='tight')
    plt.close()  # 必须 close，否则两张图的线条会叠在一起
    print(f"✅ 图表已成功保存为: result_cf_utility_{mode_str}")
...
#
# # ================= 新增：输出为 PDF =================
# # bbox_inches='tight' 确保图例和坐标轴标签不会被截断
# pdf_filename = 'evaluation_result.pdf'
# plt.savefig(pdf_filename, format='pdf',dpi=450,bbox_inches='tight')
# print(f"图表已成功保存为: {pdf_filename}")

# 如果你还需要在屏幕上预览，可以保留下面这行；
# 如果是在服务器跑脚本不需要预览，可以把它注释掉。
# plt.show()
